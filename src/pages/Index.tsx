...
import { ensemblePredict, recordEnsembleResult } from "@/lib/ensemblePredictor";
      ...
      if (frame.phase === "result" && frame.winner && now - lastResultTsRef.current > EVENT_COOLDOWN_MS) {
        ...
        setHistory(prev => {
          ...
          const newResult: RoundResult = {
            ...
            predictionHit,
            mlConfidenceBefore,
            aiPredictedBefore: null,
            aiPredictionHit: null,
            aiConfidenceBefore: 0,
            metaPredictedBefore: null,
            metaPredictionHit: null,
          };
                ...
                <div className="flex items-center gap-2">
                  ...
                  {totalRounds > 0 && (
                    <button
                      onClick={() => {
                        const rows = [
                          ["#", "Время", "Победитель", "Темп мерц.", "Смещение мерц.", "Переключений", "Прогноз ML", "ML верно", "Увер. ML %"],
                          ...history.map(r => [
                            r.id,
                      ...
                      title="Скачать историю событий в CSV"
                    >
                  ...
                  {ensemblePred && (
                    <button
                      onClick={() => {
                        ...
                        // Итог ансамбля
                        sections.push("АНАЛИЗ ML-МОДЕЛИ");
                        sections.push(`Дата;${new Date().toLocaleString("ru-RU")}`);
                        sections.push(`Всего раундов;${history.length}`);
                        sections.push(`Победитель Альфа;${history.filter(r => r.winner === "alpha").length}`);
                        sections.push(`Победитель Омега;${history.filter(r => r.winner === "omega").length}`);
                        sections.push("");

                        // Ансамбль — прогноз
                        sections.push("АНСАМБЛЬ (ПРОГНОЗ СЛЕДУЮЩЕГО)");
                        sections.push(`Прогноз;${ensemblePred.reactor === "alpha" ? "Альфа" : "Омега"}`);
                        sections.push(`Уверенность %;${Math.round(ensemblePred.confidence * 100)}`);
                        sections.push(`Лучший метод;${ensemblePred.bestMethod}`);
                        if (ensemblePred.accuracy !== null) sections.push(`Общая точность ансамбля %;${Math.round(ensemblePred.accuracy * 100)}`);
                        sections.push("");

                        // Голоса методов
                        sections.push("ГОЛОСА МЕТОДОВ");
                        sections.push("Метод;Прогноз;Уверенность %;Вес");
                        const nameMap: Record<string, string> = { markov: "Марков", lz: "LZ-суффикс", rf: "Random Forest", gbm: "GBM", knn: "kNN" };
                        for (const m of ensemblePred.methods) {
                          sections.push(`${nameMap[m.name] ?? m.name};${m.available ? (m.reactor === "alpha" ? "Альфа" : "Омега") : "нет данных"};${m.available ? Math.round(m.confidence * 100) : ""};${(ensemblePred.ensembleWeights[m.name] ?? 1).toFixed(2)}`);
                        }
                        sections.push("");

                        // Матрица ошибок
                        sections.push("МАТРИЦА ОШИБОК");
                        sections.push("Предсказано\\Факт;Альфа;Омега");
                        sections.push(`Альфа;${ensemblePred.confusionMatrix.alphaAlpha};${ensemblePred.confusionMatrix.alphaOmega}`);
                        sections.push(`Омега;${ensemblePred.confusionMatrix.omegaAlpha};${ensemblePred.confusionMatrix.omegaOmega}`);
                        if (ensemblePred.accuracyByClass.alpha !== null) sections.push(`Точность по Альфа %;${Math.round(ensemblePred.accuracyByClass.alpha * 100)}`);
                        if (ensemblePred.accuracyByClass.omega !== null) sections.push(`Точность по Омега %;${Math.round(ensemblePred.accuracyByClass.omega * 100)}`);
                        sections.push("");

                        // Важность признаков
                        if (ensemblePred.featureImportance.length > 0) {
                          sections.push("ВАЖНОСТЬ ПРИЗНАКОВ (Random Forest)");
                          sections.push("Признак;Важность %");
                          for (const f of ensemblePred.featureImportance) {
                            sections.push(`${f.label};${Math.round(f.importance * 100)}`);
                          }
                          sections.push("");
                        }

                        // Точность по блокам
                        if (ensemblePred.recentAccuracyCurve.length > 0) {
                          sections.push("ТОЧНОСТЬ ПО ВРЕМЕНИ (блоки 10 раундов)");
                          sections.push("Блок;Точность %");
                          ensemblePred.recentAccuracyCurve.forEach((v, i) => {
                            sections.push(`${i + 1};${Math.round(v * 100)}`);
                          });
                          sections.push("");
                        }

                        // Сигналы ML
                        if (signalDiagnostics.length > 0) {
                          sections.push("ТОЧНОСТЬ СИГНАЛОВ ML");
                          sections.push("Сигнал;Точность %;Образцов;Множитель");
                          for (const s of signalDiagnostics) {
                            if (s.accuracy !== null) {
                              sections.push(`${s.label};${Math.round(s.accuracy * 100)};${s.samples};${s.multiplier.toFixed(1)}`);
                            }
                          }
                        }
            ...
            {history.length === 0 ? (
              ...
              <div className="overflow-y-auto flex-1 p-3 space-y-1.5">
                ...
                <div className="flex items-center justify-end gap-2 px-1 pb-1">
                  ...
                  <span className="font-mono text-white/20 px-1.5 py-0.5 rounded text-xs" style={{ border: "1px solid rgba(251,146,60,0.3)" }}>Ансамбль</span>
                </div>
